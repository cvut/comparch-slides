...
\end{columns}
\bigskip
Přetečení při sčítání čísel se znaménkem nastane právě tehdy, když dojde k přenosu na posledních sifrách a zároveň nedojde k přenosu na předposledních cifrách.
...
Čísla s posunutou nuou:

\begin{columns}
\begin{column}{0.5\textwidth}
\begin{itemize}
\item pro k-bitouvou reprezentaci zvolíme číslo S (většinou $S=2^{k-1}$ nebo $S=2^{k-1}-1$)
\item reprezetace -- kód čísla X je $A(X) = X+S$
...
\begin{itemize}
\item[A] může dojít k přetečení jen v dvokovém doplňku
\item[B] může dojít k přetečení jen v reprezentaci s posunutou nulou
\item[C] může dojít k přetečení v dvokovém doplňku i v reprezentaci s pousunutou nulou
...
Čísla s pevnou destinnou čárkou (fixed point numbers):
...
\item reprezetace -- kód čísla X je $A(X) = \lceil X \cdot 2^s \rceil$
...
\item pak můžete nastavit $s=k$ a použí bezznaménkovou reprezentaci celých čísel
...
Počítání s čísly s pevnou destinnou čárkou:

\begin{itemize}
\item součet a rozdíl je součem a rozdílem celočíselné reprezentace čísel
...
Všiměte si, že každé reálné číslo (kromě 0) začíná v binárním vědeckém zápisu jedničkou.
...
\begin{center}
\includegraphics[width=0.5\textwidth]{float_priklad.pdf}
\end{center}
...
\smallmezi 1 a 255 & cokoliv &  nromalizovaná čísla \\ \hline
...
Reálné 16-bitové čsílo obsahuje:
\begin{itemize}
\item 1 bit znaménko 
...
\end{itemize}
\bigskip
Reálné 128-bitové čsílo obsahuje:
\begin{itemize}
\item 1 bit znaménko 
...
\item Větší expoenent větší číslo, při rovnosti exponentů větší mantisa znamená větší číslo
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{enumerate}
\item Převést mantisy čísel na společný exponent, který má hodnotu největšího expoenentu (porušíme notaci)
...
\begin{enumerate}
\item Exponent výsledku je součet expoenetů čísel
\item Mantisa výsledku je součin mantis čísel (i se skrytými jedničkami)
\item Výsledné číslo normalizovat
\begin{itemize}
\item podle výsledku násobění mantis je někdy nutné zvýšit exponent o 1 a vyrotovat mantisu doprava
\end{itemize}
\end{itemize}
\end{frame}
...
\texttt{\phantom{xxx}11} odpovídá \texttt{1.1}\vspace{-6pt}\\
\rule[0pt]{2cm}{0.4pt}\\
